...
| 文件 | 职责 |
|---|---|
| `app/main.py` | 创建 FastAPI 生命周期资源：迁移、持久 saver、hot checkpointer、ContextIndexService、Graph、Redis、SSE Registry |
| `app/checkpointing.py` | stream 级 checkpoint 热读、持久写穿透、history/time-travel 旁路与 Trace 指标 |
| `app/context_index/service.py` | LlamaIndex IngestionPipeline、HuggingFace embedding、Qdrant 归档/召回/重建 |
...
上下文状态：context_summary / session_memory / retrieved_context / context_archive_queue / context_report
...
- `Runtime[AgentRuntimeContext]` 只放缓存、ContextIndexService、搜索模式、工具额度等不可 checkpoint 的依赖。
- 父图使用 `CachedCheckpointSaver(AsyncSqliteSaver)`，`conversation_id` 是 `thread_id`，`stream_id` 是热缓存 scope。
- `chatbot.db` 保存用户真正看到的消息；`checkpoints.db` 保存 Agent 工作状态。

当业务历史最后一条消息与 checkpoint 不一致时，API 删除该 thread 的旧 checkpoint 和热值，再用业务历史重建。业务库始终是可见消息的事实来源。

### Checkpoint 为什么保留持久层，又增加 Hot Cache

`checkpoints.db` 不是普通回答缓存：它保存 super-step、父链、pending writes 和恢复位置，支持重启恢复、history 和 time-travel。只用 `InMemorySaver` 或全局字典会在进程退出时丢状态，也容易让并发请求读到旧 head。

`CachedCheckpointSaver` 因此只做一个有边界的优化：

```text
同一 Chat stream
  API graph.aget_state → SQLite miss 后写入 (stream, thread, namespace) L1
  Graph astream 启动  → 同 config hot hit，省去重复 SQLite read
  Graph aput/writes    → SQLite 先提交，再失效该 thread/namespace 所有旧 head
  Graph 结束           → 主动清理 stream scope（TTL/LRU 仅作异常兜底）

历史 / 指定 checkpoint_id / time-travel → 永远绕过 hot cache
另一个 stream / thread / namespace     → 独立 key，绝不共享对象引用
```

Trace 会记录 `checkpoint_hot_hits`、`checkpoint_durable_reads`、写入次数、读写耗时和命中率，并追加一条 `checkpoint.summary`，因此 Eval Lab 可以按代码版本回放这一优化。

## Context Engineering：五层上下文治理

该设计受 Claude Code 长会话分层治理思路启发，目标不是简单“截断到 Token 上限”，而是在保留最近对话、关键事实和完整工具协议的前提下逐级降低上下文成本。它位于 `retrieve_context` 与 `archive_context` 之间。
...
`retrieve_context` 先按最新 HumanMessage 检索，强制使用 `user_id + conversation_id + index_version` metadata filter；取 8 个候选后执行 0.35 分数阈值、节点去重、当前活动消息排除与 1,600 token 预算，最终最多保留 4 个片段。结果作为“不可信历史数据”SystemMessage 注入，同时计入 Context Manager 压力。

`context_manager` 删除完整旧 turn 时只写入 `ContextArchiveRef(start_message_id, end_message_id)`；`archive_context` 再从 `chatbot.db` 读取可信正文。索引仅保留 user/assistant 正文、来源标题和 Artifact 标题/类型，过滤 reasoning、系统提示、工具 JSON 和完整 Artifact 内容。`doc_id/node_id` 使用稳定 SHA-256，内容 hash 未变化时跳过嵌入。

Qdrant 目录默认 `./context_index`，collection 为 `chat_context_v1`，embedding 为 `BAAI/bge-small-zh-v1.5`。所有同步嵌入/Qdrant 操作放入 `asyncio.to_thread`，写入由进程级锁串行化。在线聊天默认设置 `CONTEXT_EMBED_ALLOW_DOWNLOAD=0`，只读取本机完整模型；模型尚未准备好时立即 fail-open，绝不会在 Graph 热路径等待下载。首次模型下载和索引构建由 `npm run index:context` 显式执行。

可配置项：`CONTEXT_INDEX_ENABLED`、`CONTEXT_INDEX_PATH`、`CONTEXT_EMBED_MODEL`、`CONTEXT_EMBED_ALLOW_DOWNLOAD`、`CONTEXT_INDEX_VERSION`、`CONTEXT_RETRIEVAL_TOP_K`、`CONTEXT_RETRIEVAL_MAX_CHUNKS`、`CONTEXT_RETRIEVAL_MAX_TOKENS` 和 `CONTEXT_RETRIEVAL_SCORE_THRESHOLD`。默认分别为 `1`、`./context_index`、`BAAI/bge-small-zh-v1.5`、`0`、`v1`、`8`、`4`、`1600`、`0.35`。
...
| Graph 热读/持久化 | `CachedCheckpointSaver` L1 + `checkpoints.db` | stream 级 latest 去重；完整 AgentState、history、summary、memory 持久化 |
| 历史语义索引 | LlamaIndex + 本地 Qdrant | 可重建旧 turn 向量；不保存业务真值，故障 fail-open |
...
tool_result / sources / activity / context_status / context_retrieval / context_index
```
...
`eval:checkpoint` 使用真实临时 AsyncSqliteSaver，比较每请求两次持久读取与“一次
持久读 + 一次 hot hit”，并验证 history/time-travel、写穿透失效和重启恢复。
